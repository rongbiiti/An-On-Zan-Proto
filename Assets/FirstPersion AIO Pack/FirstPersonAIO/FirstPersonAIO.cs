...
public class FirstPersonAIO : MonoBehaviourPunCallbacks
{
    ...
    public float stickRotateSpeed = 5;
    ...
    private void FixedUpdate()
    {
        ...
        if (useStamina) {
            isSprinting = Input.GetButton("Sprint") && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) > 0.01f);
            if (isSprinting) {
            ...
            } else if ((!Input.GetButton("Sprint") || Mathf.Abs(fps_Rigidbody.velocity.x) < 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) < 0.01f || isCrouching) && staminaInternal < staminaLevel) {
        ...
        } else { isSprinting = Input.GetButton("Sprint"); }
    ...
    private void OnCollisionEnter(Collision CollisionData)
    {
        for (int i = 0; i < CollisionData.contactCount; i++) {
            ...
            if (CollisionData.GetContact(i).point.y < transform.position.y - ((capsule.height / 2) - capsule.radius * 0.95f)) {
                ...
                if (advanced.maxSlopeAngle > 0) {
                    if (a < 5.1f) { advanced.isTouchingFlat = true; advanced.isTouchingWalkable = true; } else if (a < advanced.maxSlopeAngle + 0.1f) { advanced.isTouchingWalkable = true; /* IsGrounded = true; */} else if (a < 90) { advanced.isTouchingUpright = true; }
...
public class FPAIO_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (t.cameraInputMethod == FirstPersonAIO.CameraInputMethod.Traditional || t.cameraInputMethod == FirstPersonAIO.CameraInputMethod.TraditionalWithConstraints) {
            ...
            t.stickRotateSpeed = EditorGUILayout.Slider(new GUIContent("Stick RotateSpeed", "Determines how sensitive the stick is."), t.stickRotateSpeed, 1, 100);
